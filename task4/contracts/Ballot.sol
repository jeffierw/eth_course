...
contract Ballot {
    ...
    constructor(bytes32[] memory proposalNames, uint _startTime, uint _endTime) {
        ...
        // Initialize voting start and end times
    ...
    /// Allows the contract owner to set a specific voting weight for a voter
    function setVoterWeight(address voter, uint weight) external {
        require(msg.sender == chairperson, "Only the chairperson can set voting weight");
        require(block.timestamp < startTime || block.timestamp > endTime, "Cannot set weight during voting period");
        require(!voters[voter].voted, "Voter has already voted, cannot change weight");
